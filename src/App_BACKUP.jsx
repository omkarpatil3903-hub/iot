...
function App() {
  ...
  const temperatureData =
    historicalData?.temperature || mockHistorical.temperature;

  // Calculate average moisture for CWSI
  const avgMoisture = currentData
    ? ((currentData.moisture_15cm || 0) +
      (currentData.moisture_30cm || 0) +
      (currentData.moisture_45cm || 0)) /
    3
    : null;

  // Calculate real GDD from temperature history
  const calculateRealGDD = () => {
    if (!temperatureData || temperatureData.length === 0) {
      return { accumulated: 850, daily: 15 }; // Fallback to defaults
    }
    ...
    // Calculate GDD for each day in history
    temperatureData.forEach((entry, index) => {
      const temp = entry.temperature;
      if (temp != null) {
        // Simplified: Using single temp reading as average
        // In production, you'd want max/min temps per day
        const gdd = Math.max(0, temp - SUGARCANE_BASE_TEMP);
        totalGDD += gdd;

        // Use last entry for daily GDD
    ...
    return {
      accumulated: Math.round(totalGDD),
      daily: Math.round(dailyGDD)
    };
  };
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 bg-cane-green rounded-lg sm:rounded-xl shadow-lg shadow-cane-green/20">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-xs text-white/50">
                  {activeField?.name || "Field Analytics"}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                {/* App Network Status */}
                <div
                  className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-full transition-colors ${isConnected || useMockData
                ...
                {/* ESP32 Device Status */}
                <div
                  className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-full transition-colors ${isDeviceOnline
                    ? "bg-emerald-500/20 text-emerald-400"
                    : "bg-red-500/20 text-red-400"
                    }`}
                  title={
                    isDeviceOnline
                      ? "Device Online (Sending Data)"
                      : "Device Offline (Last seen > 15m ago)"
                  }
                >
                  <div
                    className={`w-2 h-2 rounded-full ${isDeviceOnline
              ...
              <button
                ...
                className="p-2 rounded-lg bg-white/5 text-white/70 hover:bg-white/10 transition-colors touch-manipulation"
              ...
              <button
                ...
                className="md:hidden p-2 rounded-lg bg-white/5 text-white/70 hover:bg-white/10 transition-colors touch-manipulation"
        ...
        {useMockData && (
          <div className="mb-6 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center gap-3 backdrop-blur-sm">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-white/50">
                Simulated data • Add Firebase credentials for live data
              </p>
            </div>
          </div>
        )}
